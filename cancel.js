...
router.post("/on_cancel", async (req, res) => {
  console.log("on_cancel body:", req.body);
  const jsonData = req.body;
  const filePath = 'on_cancel_response.txt';

  fs.writeFile(filePath, JSON.stringify(jsonData), (err) => {
    if (err) {
      console.error('Error writing to file:', err);
    } else {
      console.log('Data has been written to the on_cancel file successfully!');
    }
  });
...
router.post("/cancel", async (req, res) => {
  ...
  var url = `${cleanedBppUri}/${strings.cancel}`;
