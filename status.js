...
router.post("/on_status", async (req, res) => {
  console.log("on_status body:", req.body);
  const jsonData = req.body;
  const filePath = 'on_status_response.txt';
  ...
  if (!jsonData.context || !jsonData.context.message_id) {
    ...
    fs.writeFile(filePath, JSON.stringify(jsonData), (err) => {
        if (err) {
          ...
          console.log('Data has been written to the on_status ${} file successfully!');
        }
      });
  }
  if(jsonData.message){
